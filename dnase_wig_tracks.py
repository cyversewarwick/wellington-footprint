...
for each in progress.bar([item for sublist in sorted(regions.intervals.values()) for item in sorted(sublist, key=lambda peak: peak.startbp)]):
    ...
    fwigout.write("fixedStep\tchrom=" + str(each.chromosome) + "\t start="+ str(each.startbp+1) +"\tstep=1\n")
    for i in f:
        fwigout.write(str(i)+'\n')
    bwigout.write("fixedStep\tchrom=" + str(each.chromosome) + "\t start="+ str(each.startbp+1) +"\tstep=1\n")
    for i in r:
        if args.real:
            bwigout.write(str(i)+'\n')
        else:
